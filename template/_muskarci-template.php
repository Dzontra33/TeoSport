...
$product_shuffle = $product->getProductsBySex(1);
...
if (isset($_GET['btnFiltriraj'])) {
    if (isset($_GET['catID']) && !empty($_GET['catID']) && isset($_GET['brandID']) && !empty($_GET['brandID'])) {
        ...
        $in = implode(", ", $niz);
        $cat_id = $_GET['catID'];
        $product_shuffle = $product->getProductsByCatAndBrand(1, $cat_id, $in);
    } else if (isset($_GET['catID']) && !empty($_GET['catID'])) {
        $product_shuffle = $product->getProductsByCat(1, $_GET["catID"]);
    } else if (isset($_GET['brandID']) && !empty($_GET['brandID'])) {
        $niz = $_GET['brandID'];
        $in = implode(", ", $niz);
        $product_shuffle = $product->getProductsByBrand(1, $in);
    }
}
...
<section id="product-section">
    ...
    <img src="./assets/img/muskarci-banner.jpg" class=" img-fluid" alt="...">
    </div>
    <div class="container">
        <div class="row">
