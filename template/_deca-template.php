...
$product_shuffle = $product->getProductsBySex(3);
...
if (isset($_GET['btnFiltriraj'])) {
    if (isset($_GET['catID']) && !empty($_GET['catID']) && isset($_GET['brandID']) && !empty($_GET['brandID'])) {
        ...
        $in = implode(", ", $niz);
        $cat_id = $_GET['catID'];
        $product_shuffle = $product->getProductsByCatAndBrand(3, $cat_id, $in);
    } else if (isset($_GET['catID']) && !empty($_GET['catID'])) {
        $product_shuffle = $product->getProductsByCat(3, $_GET["catID"]);
    } else if (isset($_GET['brandID']) && !empty($_GET['brandID'])) {
        $niz = $_GET['brandID'];
        $in = implode(", ", $niz);
        $product_shuffle = $product->getProductsByBrand(3, $in);
    }
}
...
<section id="product-section">
    <div class="card text-center mb-3 border-0">
        <img src="./assets/img/deca-banner.jpg" class=" img-fluid" alt="...">
    </div>
    <div class="container">
        <div class="row">
